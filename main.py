...
def is_hevc(file_path):
    ...
    """
    try:
        codec_info = subprocess.check_output(["ffprobe", "-v", "error", "-select_streams", "v:0", "-show_entries", "stream=codec_name", "-of", "csv=p=0", str(file_path)])
...
def transcode_to_hevc(input_file):
    ...
    subprocess.run([
        "ffmpeg", "-i", input_path, "-c:v", "libx265", "-crf", "23", "-preset", "medium",
        "-vf", "scale=min(iw\\,1920):-2",  # Limit resolution to 1080p
        "-c:a", "aac", "-b:a", "128k",
        "-movflags", "+faststart",  # Enable faststart for streaming
        "-pix_fmt", "yuv420p",  # Optimal pixel format for compatibility
        "-profile:v", "main",  # Main profile for wider compatibility
        "-level", "4.0",  # Level 4.0 for compatibility with Google TV
        output_path
...
def main(target_directory, dry_run=False):
    ...
    video_files = [file for file in target_directory.glob('**/*.*') if file.suffix.lower() in video_extensions]
    ...
    if dry_run:
        ...
        logger.info(f"{transcode_count}/{total_files} files would be transcoded ({(transcode_count / total_files) * 100:.2f}%).")
        return
    ...
    logger.info(f"Transcoding complete. {transcode_count}/{total_files} files transcoded.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Batch transcode video files to HEVC.")
    parser.add_argument("target_directory", help="Target directory to search for video files.")
    parser.add_argument("--dry-run", action="store_true", help="Preview the number of files to be transcoded (no actual transcoding)")
